...
| `permissions` | AWS, GCP | Implemented in AzureFox | `permissions` | `Azure analogue` | AzureFox now provides subscription-scoped permission triage that highlights high-impact RBAC roles, assignment volume, and current-identity exposure; deeper effective-permission expansion can iterate later. | Privilege analysis | Lab planned |
| `cape` / `privesc` | AWS, GCP | Implemented in AzureFox | `privesc` | `Covered differently` | AzureFox now triages Azure-native escalation leads by combining high-impact RBAC roles with managed-identity and public-workload context rather than AWS-style role assumption semantics. | Privilege escalation pathing | Lab planned |
| `role-trusts` / `identity-federation` | AWS, GCP | Implemented in AzureFox | `role-trusts` | `Covered differently` | AzureFox now triages app-registration, service-principal, federated-credential, ownership, and app-role assignment trust edges as an operator-first trust review surface rather than AWS-style role assumption semantics. | Trusted relationship abuse | Lab planned |
